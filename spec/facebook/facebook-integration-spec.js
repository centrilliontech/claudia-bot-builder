  ...
  describe('API integration wiring', () => {
    describe('token verification', () => {
      it('uses the text/plain content type', () => {
        expect(
          underTest.apiConfig().routes.facebook.GET.success.contentType
        ).toEqual('text/plain')
      })
        ...
        underTest
          ...
          .then(() => {
            expect(lambdaContextSpy.done).toHaveBeenCalledWith(
              null,
              jasmine.objectContaining({ body: 'XHCG' })
            )
          })
          .then(done, done.fail)
      })
      ...
      it('returns Error challenge if the tokens do not match', done => {
        underTest
          ...
          .then(() => {
            expect(lambdaContextSpy.done).toHaveBeenCalledWith(
              null,
              jasmine.objectContaining({ body: 'Error' })
            )
          })
          .then(done, done.fail)
      })
        ...
        https.request.pipe(callOptions => {
          expect(callOptions).toEqual(
            jasmine.objectContaining({
              ...
              path: '/v3.1/me/messages?access_token=12345',
              protocol: 'https:',
